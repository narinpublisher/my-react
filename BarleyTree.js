	...
	render(){
		return (
			<section>
				...
				<p>
					<b className='aqua'>{ this.state.mun }</b>
					{ this.state.communism }
					{ this.state.eradicate }<br />
					나는 그 그늘 아래 단 꿈을 보았네<br />
					가지에 희망의 말 새기어 놓고서<br />
					기쁘나 슬플 때나 찾아온 나무 밑
				</p>
 				<button type='button' onClick={this.change}>텍스트 변경하기</button>
			</section>
		);
	}
}
